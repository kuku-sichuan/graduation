...
class RNN_lstm(object):
    def __init__(self,input_dims,time_steps,num_hidden1, num_hidden2,num_stack,num_class,learning_rate,batch_size,nn_type,train):
    ...
    def brnn_network(self):
        with tf.name_scope('b_rnn'):
            ...
            lstm_fw_cell = tf.contrib.rnn.BasicLSTMCell(self.num_hidden1, forget_bias=1.0,state_is_tuple=True)
            # Backward direction cell
            lstm_bw_cell = tf.contrib.rnn.BasicLSTMCell(self.num_hidden1, forget_bias=1.0,state_is_tuple=True)
    ...
    def rnn_network(self):
        with tf.name_scope('rnn'):
            ...
            cell = tf.contrib.rnn.BasicLSTMCell(self.num_hidden1, forget_bias=1.0, state_is_tuple=True)
            ...
            tf.summary.histogram('rnn/' + 'hid_val/' + 'pre_normal', outputs)
            with tf.name_scope('batch_normal'):
                ewma = tf.train.ExponentialMovingAverage(decay=0.99)
                bnrnn = BatchNormalizer(self.num_hidden1, 1e-5, ewma, True)
                update_assignment2 = bnrnn.get_assigner()
                hidden1 = bnrnn.normalize(out,train=self.train)
                tf.summary.histogram('after_normal',hidden1)
            return hidden1
        ...
        with tf.name_scope('full_1'):
            if self.nn_type == 'rnn':
                with tf.name_scope('weights'):
                   weights = self.weight_init([self.num_hidden1,self.num_hidden2])
                ...
                with tf.name_scope('weights'):
                    weights = self.weight_init([2 * self.num_hidden1, self.num_hidden2])
                    self.variable_summaries(weights, 'w')
                with tf.name_scope('biases'):
                    biases = self.bias_init([self.num_hidden2])
                    self.variable_summaries(biases, 'b')
            ...
            with tf.name_scope('batch_normal'):
                ewma = tf.train.ExponentialMovingAverage(decay=0.99)
                bn = BatchNormalizer(self.num_hidden2, 1e-5, ewma, True)
                update_assignment3 = bn.get_assigner()
                hidden2 = bn.normalize(hidden2,train=self.train)
                tf.summary.histogram('after_normal',hidden2)

        with tf.name_scope('full_2'):
                with tf.name_scope('weights'):
                   weights = self.weight_init([self.num_hidden2,self.num_class])
                   self.variable_summaries(weights, 'w')
                with tf.name_scope('biases'):
                   biases = self.bias_init([self.num_class])
                   self.variable_summaries(biases,'b')
                pre = tf.matmul(hidden2, weights) + biases  # size is (N * T) * 2
